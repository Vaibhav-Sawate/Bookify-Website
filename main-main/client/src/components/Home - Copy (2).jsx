  ...
  useEffect(() => {
    if (!allAudiobooks) {
      getAllAudiobooks().then((data) => {
        ...
        // Create a mapping of series to audiobooks
        const seriesMapping = {};
        data.data.forEach((audiobook) => {
          const series = audiobook.series || "Other";
          if (!seriesMapping[series]) {
            seriesMapping[series] = [];
          }
          seriesMapping[series].push(audiobook);
        });
...
<div className="w-full h-auto flex items-center justify-evenly gap-4 flex-wrap p-4">
        {Object.keys(seriesMap).map((series) => (
          ...
          >
           <NavLink to={`/series/${encodeURIComponent(series)}`}>
              <p className="text-base text-headingColor font-semibold my-2">{series}</p>
            </NavLink>
          </motion.div>
        ))}
                 <HomeAudiobookContainer musics={filteredAudiobooks} />
      </div>
